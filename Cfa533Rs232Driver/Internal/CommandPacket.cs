...
namespace Petrsnd.Cfa533Rs232Driver.Internal
{
    internal class CommandPacket
    {
        ...
        public byte[] ConvertToBuffer()
        {
            ...
            Crc = CalculateCrcFromTable(buffer);
            buffer[PacketSize] = (byte)(Crc & 0xff); // lower CRC byte
            buffer[PacketSize + 1] = (byte)(Crc >> 8); // upper CRC byte
            return buffer;
        }
        ...
        public int PacketSizeWithCrc => PacketSize + sizeof (ushort);
        ...
        static readonly ushort[] CrcLookupTable = new ushort[]
        {
        ...
        private ushort CalculateCrcFromTable(byte[] buffer)
        {
            // C# Conversion of algorithm
            ushort newCrc = 0xFFFF;
            var index = 0;
            var len = PacketSize;
            while (len > 0)
            {
                var lookup = (newCrc ^ buffer[index]) & 0xff;
                newCrc = (ushort)((newCrc >> 8) ^ CrcLookupTable[lookup]);
                index++;
                len--;
            }
